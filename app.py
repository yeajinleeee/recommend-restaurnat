...
def weather_group_from_id(weather_id: int) -> str:
    ...
    return "구름"
def recommended_categories_from_group(group_name: str, top_k=None):
...
def filter_by_category_tf(frame: pd.DataFrame, theme: str) -> pd.DataFrame:
    ...
    frame=coerce_tf_bool(frame); col_name=resolve_tf_column(frame,theme)
...
def select_and_filter_by_business_type(frame: pd.DataFrame):
...
def main():
    ...
    try:
        w=fetch_weather(user_lat,user_lon); group_name=weather_group_from_id(w["id"]); opts,mood=recommended_categories_from_group(group_name)
    except: w={"description":"알수없음","temperature":"?"}; group_name,opts,mood="구름",["가볍게 간단히","든든한 한끼","디저트/카페"],"실내 중심"

    with st.sidebar:
        st.markdown(f"<div style='background:#fff; padding:15px; border-radius:10px; margin-bottom:10px;'>📍 위도:{user_lat:.4f}, 경도:{user_lon:.4f}</div>",unsafe_allow_html=True)
        st.markdown(f"<div style='background:#fff; padding:15px; border-radius:10px; margin-bottom:10px;'>🌤️ {w['description']} {w['temperature']}°C</div>",unsafe_allow_html=True)
        st.markdown(f"<div style='background:#fff; padding:15px; border-radius:10px;'>💡 {mood}</div>",unsafe_allow_html=True)
    ...
    if st.session_state.page=="page1":
        ...
        st.subheader(f"‘{choice}’ 카테고리에 해당되는 음식점 (거리순)")
        ...
        with col2:
            if st.button("➡ 다음"):
                st.session_state.choice=choice; st.session_state.page="page2"; st.rerun()
    ...
    # Page 2
        ...
        df=prettify_dataframe(filtered).reset_index(drop=True)
        df.index=df.index+1

        # 행 선택 기능
        selected_row=st.data_editor(
            df[["이름","거리","별점","리뷰 수"]].fillna(""),
            hide_index=False, disabled=True, use_container_width=True, height=500,
            key="select_table"
        )
        ...
        if isinstance(selected_row,pd.DataFrame) and not selected_row.empty:
            ...
            st.session_state["selected_store"]=filtered.iloc[row_idx].to_dict()
    ...
    # Page 3
        ...
        if store:
            st.subheader(store.get("이름","알 수 없음"))
            st.write("업태:",store.get("category","정보 없음"))
            st.write("별점:",store.get("rating","정보 없음"))
            st.write("리뷰 수:",store.get("review_cnt","정보 없음"))
            if store.get("map_link"):
                st.markdown(f"[📍 지도 보기]({store['map_link']})",unsafe_allow_html=True)
        else: st.warning("선택된 식당이 없습니다.")
