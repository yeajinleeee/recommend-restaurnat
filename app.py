...
def main():
    ...
    if st.session_state.page == "page1":
        ...
        if not filtered_df.empty:
            st.dataframe(filtered_df[["name_g","distance_m"]], use_container_width=True, height=500)
        else:
    ...
    # Page 2
        ...
        with tabs[0]:
            st.dataframe(filtered.sort_values("distance_m")[["name_g","distance_m"]])
        with tabs[1]:
            if "rating" in filtered.columns:
                st.dataframe(filtered.sort_values("rating", ascending=False)[["name_g","rating"]])
        with tabs[2]:
            if "review_cnt" in filtered.columns:
                st.dataframe(filtered.sort_values("review_cnt", ascending=False)[["name_g","review_cnt"]])
        with tabs[3]:
