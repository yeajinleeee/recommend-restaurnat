...
# ---------------------------------
# 사이드바와 메인 콘텐츠의 디자인을 위한 CSS 코드입니다.
IMPROVED_UI_CSS = """
<style>
...
WX_RECO = { "클리어": {"mood": "야외활동, 기분전환", "cats": ["이국적인 음식","디저트/카페","술 한잔 하기 좋은 날","가볍게 간단히","시원한 음식","해산물/생선요리"]}, 
...
def weather_group_from_id(weather_id: int) -> str:
    """OpenWeatherMap의 날씨 ID를 내부 그룹명('맑음', '비' 등)으로 변환합니다."""
...
def recommended_categories_from_group(group_name: str):
    """날씨 그룹명을 받아 추천 카테고리 리스트와 분위기 설명을 반환합니다."""
...
def get_restaurant_within_500m_from_supabase(lat: float, lon: float):
    """Supabase DB에 RPC 요청을 보내 반경 500m 내 식당 목록을 가져옵니다."""
    try:
...
def filter_by_category_tf(frame: pd.DataFrame, theme: str) -> pd.DataFrame:
    """사용자가 선택한 특정 카테고리에 해당하는 식당만 필터링하고 거리순으로 정렬합니다."""
...
def select_and_filter_by_business_type(frame: pd.DataFrame) -> Tuple[pd.DataFrame, List[str]]:
    """multiselect UI를 렌더링하고, 사용자가 선택한 업태로 식당 목록을 추가 필터링합니다."""
...
def pick_one_restaurant(frame: pd.DataFrame) -> Tuple[pd.Series | None, str]:
    if frame is None or frame.empty: return None, ""
    
    def format_func(index):
        row = frame.loc[index]
        return f"{row.get('name', '')} ({format_distance(row.get('distance_m'), 'm')})"

    selected_index = st.selectbox("하나를 골라보세요!", index=frame.index, format_func=format_func)
    if selected_index is None: return None, ""
    return frame.loc[selected_index], format_func(selected_index)
