...
# 4. UI Helper
# ───────────────────────────────
def render_scroll_table(frame: pd.DataFrame, cols_to_show: List[str]) -> pd.DataFrame:
    if frame.empty:
...
def select_and_filter_by_business_type(frame: pd.DataFrame) -> Tuple[pd.DataFrame, List[str]]:
    if frame.empty or "category" not in frame.columns:
        return frame, []
    cats_all = (
        frame["category"].dropna().astype(str).str.strip().unique().tolist()
    )
...
def main():
    ...
    if st.session_state.page == "page1":
        ...
        if choice in all_df.columns:  # 불리안 칼럼 사용
