...
def fetch_weather(lat:float, lon:float)->dict:
    ...
    res=requests.get(url,timeout=10); res.raise_for_status(); d=res.json()
...
def get_restaurant_within_500m_from_supabase(lat:float, lon:float):
    try:
        ...
        if not res or res.data is None or len(res.data)==0: return pd.DataFrame()
...
def prettify_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "distance_m" in df.columns:
        ...
        df["거리"]=pd.to_numeric(df["distance_km"],errors="coerce").apply(lambda x:f"{x:.2f}km" if pd.notna(x) else "")
    rename_map={"name":"이름","place_name":"이름","store_name":"이름","상호명":"이름",
...
def render_paginated_clickable_name_table(frame:pd.DataFrame,*,table_key:str,page_size:int=10)->pd.DataFrame:
    if frame is None or frame.empty:
        st.info("표시할 식당이 없습니다."); return pd.DataFrame()
    df=frame.copy()
...
def main():
    ...
    pretty_all=prettify_dataframe(all_df)
    _=render_paginated_clickable_name_table(pretty_all,table_key="all_df")

    st.subheader(f"‘{choice}’ 카테고리에 해당하는 식당")
    filtered=prettify_dataframe(all_df[all_df.get(choice,False)==True]) if choice in all_df.columns else pd.DataFrame()
    _=render_paginated_clickable_name_table(filtered,table_key="filtered_choice")
